...
if [[ "$CREATE_VENV" == "true" ]]; then
    print_header "Setting up Conda Environment (using Micromamba)"
    
    # Non-interactive shells don't source .bashrc, so micromamba may not be in PATH
    # Check common installation locations and add to PATH if needed
    if ! command -v micromamba &> /dev/null; then
        for micromamba_path in \
            ...
            "/opt/micromamba/bin/micromamba"; do
            if [[ -x "$micromamba_path" ]]; then
                export PATH="$(dirname "$micromamba_path"):$PATH"
    ...
    # Final check
    if ! command -v micromamba &> /dev/null; then
        echo "Error: micromamba is not installed or not found in common locations"
        echo "Please install micromamba or add it to PATH before running this script"
    ...
    # Note: micromamba env list output has leading spaces, so we can't use ^
    if micromamba env list | grep -qE "^\s*${CONDA_ENV_NAME}\s"; then
        print_step "Conda environment '$CONDA_ENV_NAME' already exists"
        read -p "Do you want to recreate it? [y/N] " -n 1 -r
        echo
        if [[ $REPLY =~ ^[Yy]$ ]]; then
            print_step "Removing existing environment..."
            micromamba env remove -n "$CONDA_ENV_NAME" -y
            print_step "Creating new conda environment '$CONDA_ENV_NAME' with Python $PYTHON_VERSION..."
            micromamba create -n "$CONDA_ENV_NAME" python="$PYTHON_VERSION" -y
        fi
    else
...
if [[ "$INSTALL_DEPS" == "true" ]]; then
    ...
    print_step "Installing nvte"
    # Clone TE repo and submodules
    if [[ -d "TransformerEngine" ]]; then
        echo "TransformerEngine directory already exists, skipping clone"
    else
    ...
    git checkout v2.2_rocm
    # Update submodules after branch checkout (needed for hipify_torch, etc.)
    ...
    # Install torchdata with --no-deps to avoid pulling in torch from PyPI
    # (we already built torch from source)
    pip install --no-deps torchdata
    # Install remaining requirements, excluding torchdata
    grep -v "^torchdata" requirements.txt | pip install -r /dev/stdin
...
if command -v micromamba &> /dev/null; then
    eval "\$(micromamba shell hook --shell bash)"
    micromamba activate "\$CONDA_ENV_NAME" 2>/dev/null || echo "Warning: Could not activate conda env '\$CONDA_ENV_NAME'"
fi
